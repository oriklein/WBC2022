	...
	function callbackdesc2(res)
	{
		if (isError(res)) return false;
		alert(res.value);
		//window.location.href("edit_attendee.aspx?eid=16&rsid=86219");
		var rsid=document.getElementById('ctl00_CPHPageContent_HdnResID').value
		var summid=document.getElementById('ctl00_CPHPageContent_HdnSumID').value

			var browser=navigator.appName;
			if(browser.indexOf("Microsoft") >= 0)
			{
				//window.location.href("EditProduct.aspx?Productid="+d1[1]);
				window.location.href("edit_attendee.aspx?eid="+summid+"&rsid="+rsid);
			}
			else
			{
				//location.replace("EditProduct.aspx?Productid="+d1[1]);
				location.replace("edit_attendee.aspx?eid="+summid+"&rsid="+rsid);
			}
	...
	function Update(i,s,Qualifier,j,k)
	{
		...
		else
		{
			...
			if(document.getElementById('ctl00_CPHPageContent_HdnPh'+(j-1)).value=="N")
			phext="N";
			else
			phext="";
		}
		...
		var AttType="A";
		//alert(eid);
		SeminarBuynow.UpdateAttendeeInfo(i,fname,lname,org,"",eid,"",phext,spneed,ResID,AttType,Address,City,State,Zip,child,parseInt(age),PMF,PASS,callbackdesc1);
		return false;
	}
	function Delete(j)
	{
		if (confirm("Do you want to delete? "))
		{
			...
			else
			{
				...
				var Rsid=document.getElementById('ctl00_CPHPageContent_HdnResID').value;
				SeminarBuynow.DeleteAttendeeInfo(j,Rsid,callbackdesc2);
				//var rsid=document.getElementById('ctl00_CPHPageContent_HdnResID').value
		       		//var summid=document.getElementById('ctl00_CPHPageContent_HdnSumID').value
		       		//alert(rsid);
		       		//alert(summid)
